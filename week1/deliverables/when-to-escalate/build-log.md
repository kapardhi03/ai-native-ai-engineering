...
| Q4 | Reasons for rows `0c`, `0e`, `3`, `10`, `13`, `14`, `17`, `18`, `20`, `22` and `24` are still blank. | Open | Fill in above. |
...
| Q10 | No synthetic case set exists yet in `data/`, so nothing has been run against a real LLM. Decision 15 is enforced but never exercised against a live API. | Open | Next proposed step; needs design input on case count, coverage and whether cases carry labels. |
| Q11 | Nothing downstream of the belief exists yet: no cost matrix, no policy, no baseline. The locked design's decision rule (0b) and hard constraint (0c) are unimplemented. | Open | After the case set. |
